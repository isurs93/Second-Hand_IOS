...
protocol FavList: class {
    func favListloaded(items:NSArray)
}

class BoardFav: NSObject{
    var delegate: FavList!
    let urlPath = "\(coreIP)Board_Fav.jsp?mySeqno=\( user_info.user_Seqno!)"
...
//            print("MySQLForMapPageModel의 parseJSON 출구")
            ...
            self.delegate.favListloaded(items: boardList)
        })
    }
